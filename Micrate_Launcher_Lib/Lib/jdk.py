...
def install(
        ...
        jre: bool = False,
        Callback=None,
        JRE_DIR = path.join(_USER_DIR, '.jre'),
        _JDK_DIR = path.join(_USER_DIR, '.jdk')) -> str:
    if Callback == None:
        Callback = {"setStatus": empty,
                    "setMax": empty,
                    "setProgress": empty,
                    "Finish": empty}
    Callback["setStatus"]("Get Url")
    Callback["setMax"](4)
    ...
    try:
        ...
        Callback["setStatus"]("Get Java File")
        Callback["setProgress"](1)
        jdk_file = _download(url)
        Callback["setStatus"]("Get extension File")
        Callback["setProgress"](2)
        jdk_ext = _get_normalized_compressed_file_ext(jdk_file)
        Callback["setProgress"](3)
        Callback["setStatus"]("Decompress Archive")
