...
def upload_skin(uuid_player, access_token, path, slim=False):
    headers = {
    ...
    response = requests.put("https://api.mojang.com/user/profile/{uuid}/skin".format(uuid=uuid_player),
                            headers=headers,
                            files=files)
    return response


def reset_skin(uuid_player, access_token):
    headers = {
        "Authorization": "Bearer " + access_token
    }
    response = requests.delete("https://api.mojang.com/user/profile/{uuid}/skin".format(uuid=uuid_player),
                               headers=headers)
    return response
